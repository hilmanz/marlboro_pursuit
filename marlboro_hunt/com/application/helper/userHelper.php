...
class userHelper {
	...
	function getHuntPlayer()
	{
		$sql = "SELECT * FROM social_member WHERE verified = 1 AND n_status = 1";
	...
	function updateDataUser()
	{
		...
		*/
		...
		if ($this->apps->_p('changepasswd')){
			if ($oldpasswd !=''){
				
				$password = $rs['password'];
				$check = sha1($rs['salt'].$oldpasswd);
				
				if ($password == $check){
					if($newpasswd == $confirmpasswd){
						$password = sha1($rs['salt'].$newpasswd);
						$status = 1;
					}
				}else{
					// password tidak sama
		...
		// $barangay = trim(strip_tags($this->apps->_p('barangay')));
		...
		$mobile_number = $pre_mobile.'-'.$last_mobile;
		// $landline_number = $pre_line.'-'.$last_line;
		
		($city == 0) ? $cityID = $userSess->city : $cityID = $city;
		
		// pr($userSess);
		$sql = "UPDATE social_member SET nickname = '{$nickname}', sex = '{$sex}', phone_number = '{$mobile_number}',
				password = '{$password}', StreetName = '{$house}', city = '{$cityID}', zipcode = '{$zipcode}', refered_by = '{$refered_by}'
	...
	function saveUserBrands()
	{
		...
		$brand_primary = trim($this->apps->_p('brand_primary'));
		$brand_secondary = trim($this->apps->_p('brand_secondary'));
		$question_mark = trim($this->apps->_p('question_mark'));
